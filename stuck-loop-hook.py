...
def main():
    ...
    if blocks >= ESCALATE_AT - 1:
        escalate = (
            "\n\n[격상 게이트] 이 세션에서 스턱루프 블록을 이미 %d번 냈습니다. "
            "방법을 바꾸고도 또 막혔다는 뜻입니다. 그래서 블록 마지막 줄에 아래 문장을 "
            "그대로 쓰세요. 필요한지 따지지 말고 씁니다.\n"
            "  모델 바꾸기 권함: 더 강한 모델로 바꿔 보길 권해요. 지금까지 대화는 그대로 "
            "이어지고, 계속 헤매는 쪽이 더 손해예요.\n"
            "  이미 가장 강한 모델이면 이 문장 대신 «새 세션에서 처음부터 다시» 또는 "
            "«사용자가 직접 확인»을 권하세요. 이 줄은 빼지 마세요."
            % blocks
        )
    else:
        escalate = ""
    ...
    emit_tripwire(
        ...
        "  다른 방법: {지금 방식이 왜 원리적으로 안 되는지 한 줄 + 대안 A / B / C 장단점 한 줄씩}\n"
        "  외부 교차검증: {이 세션 두 번째 스턱루프부터 판단 없이 실행 — kairos 스킬 crosscheck.py --mode code. 첫 번째면 생략}\n"
