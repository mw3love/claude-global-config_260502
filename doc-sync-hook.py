...
def global_reminder(push_cwd):
    ...
    try:
        ...
        return (
            ...
            "- 변경 파일(%d개):\n  - %s\n\n"
            "처리 지시(CLAUDE.md 규칙 10-e): 아래 예외가 아니면 스스로 판단해 커밋(+push)하거나 "
            "필요하면 .gitignore에 추가해 정리하세요. 예외 — 파일 삭제가 필요해 보임 / 내용에 "
            "토큰·비밀번호·키 등 민감정보로 의심되는 게 있음 / push가 원격에 뒤처져(fast-forward "
            "불가) 거부됨 → 이 경우만 자동 처리하지 말고 사용자에게 물어보세요(강제 push 금지)."
            % (len(changed), sample)
        )
    except Exception:
        return None
