...
RULES = [
    ...
    # ── 2026-07-11 저녁: 10-b 블록의 내부 형식. '[세션 상태]'가 발화한 뒤
    #    그 안이 규격대로인지를 잰다. 세션 상태 발화 수보다 적으면 형식이 새는 것.
    #    (형식은 그날 여러 번 바뀌었다: 세션 상태: → [세션 상태] → `[세션 상태]` 백틱+브라켓.
    #     지문은 백틱을 선택적으로 허용해 최종형까지 잡는다.)
    # 2026-08-11 재감사에서 0으로 나와 '사문화'로 오판할 뻔했다 — 실제로는 CLAUDE.md
    # 규칙 10-a가 그 사이 이 라벨을 폐기하고 '[지금] 커밋하기' 항목으로 흡수했다
    # (커밋 4a7e7a2에서 도입 → 8e33746에서 대체). 지문을 최신 표현으로 교체.
    ("규칙10-a '[지금] 커밋하기' 항목",  "2026-07-11", ("re", r"\[지금\]`?\s*커밋하기")),
    ("  └ 권장 항목 '[프롬프트]' 라벨",  "2026-07-11", ("re", r"^\s*`?\[프롬프트\]`?")),
...
def main():
    ...
    earliest_added = min(added for _, added, *_ in RULES)
    if sessions and min(sessions) >= earliest_added:
        print(f"⚠ 세션 로그가 {min(sessions)}부터만 남아 있다 — 가장 오래된 규칙 추가일"
              f"({earliest_added})보다 늦다. '추가 前' 값은 전부 0으로 나오는데, 이건 규칙이"
              f" 그때부터 생겼다는 뜻이 아니라 **그 이전 세션 기록 자체가 소실**됐다는 뜻이다"
              f"(2026-08-11 재감사 때 실측: 173세션 원본이 사라지고 68세션만 남음)."
              f" 이후 재감사는 '추가 前/後' 비교를 버리고 절대 발화율(세션당)로만 판단할 것.\n")
