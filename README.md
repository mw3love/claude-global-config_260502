...
- **post-merge hook** — `git pull` 후 환경 자동 점검·복구 (Bun 설치, $PROFILE 갱신, 플러그인 다운로드, memory 연결, Cursor 훅 링크)
- **Cursor** — 같은 전역 스킬(`~/.claude/skills`)을 Cursor가 호환 로딩한다. Cursor 전역 훅 원본은 `cursor-hooks/`에 두고, post-merge가 `~/.cursor/hooks.json`으로 연결한다. Cursor용 별도 git 없음. `git push` 문지기(doc-sync 센티널)는 Claude와 같은 `.doc-sync-ready`를 쓴다.
