...
function Get-GlobalReminder([string]$PushCwd) {
    try {
        ...
        return "[global-reminder] 방금 다른 프로젝트에서 push했는데, 전역 저장소(~/.claude)에도 memory/ 외의 미반영 변경(미커밋 또는 미푸쉬)이 있습니다.`n- 변경 파일($($changed.Count)개):`n  - $sample`n`n처리 지시: 아래 예외가 아니면 스스로 판단해 커밋(+push)하거나 필요하면 .gitignore에 추가해 정리하세요. 예외 - 파일 삭제가 필요해 보임 / 내용에 토큰·비밀번호·키 등 민감정보로 의심되는 게 있음 / push가 원격에 뒤처져(fast-forward 불가) 거부됨 -> 이 경우만 자동 처리하지 말고 사용자에게 물어보세요(강제 push 금지)."
    }
    catch { return $null }
}
