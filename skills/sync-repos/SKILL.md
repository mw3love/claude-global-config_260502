...
- **[2026-08-06] 바탕화면 리포트 상태별 표 분리** — 한 표에 전 레포를 몰아넣으면 업데이트·문제 항목이 변경없음 사이에 묻히던 문제(사용자 피드백). 맨 위 불릿 요약(업데이트/미클론/문제/변경없음 건수) 뒤에 상태별(업데이트/업데이트+빌드/미클론/pull 실패/그 외) 표를 따로 두고, 변경없음 표는 가장 덜 중요한 정보라 항상 맨 아래.
