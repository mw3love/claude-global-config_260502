...
> 마지막 업데이트: 2026-09-16 (기안문 항목 구조에 "유형 C: 자체시행 완료보고" 추가 — 계약정보 없는 자체인력 경미보수 완료보고용)
...
### 유형 C — 자체시행 완료보고 (계약·용역 없이 총국 자체인력이 마친 경미한 보수·정비)

참고: `References/260916 모악산(송) 주변환경정리 완료보고(자체시행 완료보고형 양식 예시).html`
...
2. 작업 개요  ← 표로 구현 (구분/내용/비고 — 작업기간·작업장소·시행방법 등. 계약정보·예산과목은 없음)
...
- 계약업체·계약금액·예산과목 등 계약 정보 항목은 넣지 않는다(유형 A의 준공보고와 구분점). 자체인력 시행이므로 있을 수 없는 정보다.
- 관련근거는 규칙 6-b대로 별도 확인 후 있으면만 추가한다.
- 전후 비교 사진이 있으면 PDF 등으로 별도 첨부 자료를 만들어 첨부 목록에 올린다(사진은 기안문 HTML 안에 직접 삽입하지 않는다).
