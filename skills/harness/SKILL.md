...
### Phase 0-0: 적합성 사전심사 (게이트)

하네스 스킬이 트리거되면, 본격 구축에 들어가기 전에 **이 요청이 하네스를 쓸 만한 작업인지부터 판단한다.** 하네스는 반복적·다단계 협업을 자동화하는 메타 도구라 1회성 단순 작업에는 과하다. 이 게이트의 목적은, 사용자가 적합성을 스스로 판단하기 어려울 때 스킬이 **일관되게** 걸러주는 것이다. 모델의 즉흥 판단에 맡기지 말고 이 절차를 따른다.

**판단 기준 — 다음 4축을 따져본다:**
- **반복성**: 같은 유형의 작업을 앞으로 여러 번 반복할 것인가? (1회성이면 부적합 신호)
- **협업 구조**: 2명 이상의 전문 에이전트가 실시간 조율·중간 산출물 교환을 해야 하는가?
- **다단계성**: 생성→검증→통합처럼 단계가 나뉘고 단계 간 의존이 있는가?
- **재사용 가치**: 만들어 둘 에이전트·스킬을 다음 세션에서도 다시 쓸 것인가?

**분기:**
- **다수 축에 해당** → 하네스 구축 진행. 아래 Phase 0(현황 감사)으로 이동한다.
- **1회성·단순·단일 작업에 가까움** → **구축을 멈춘다.** 하네스가 과한 이유와 직접 처리 방안(어떤 도구·접근으로 바로 해결할지)을 사용자에게 제시하고 확인을 받는다. 사용자가 그래도 구축을 원하면 진행한다.
- **모호하면** → 어느 쪽인지 사용자에게 1~2개 질문으로 확인한 뒤 분기한다. 조용히 한쪽으로 가정하지 않는다.

> 이 게이트는 "하네스 점검/감사/현황" 같은 **기존 하네스 운영·유지보수 요청에는 적용하지 않는다** — 그 경우는 이미 하네스가 존재하므로 곧장 Phase 0으로 간다.
